...
do
	...
	do
		...
		echo "Picked 2 cores from CCD: $firstCCD and Picked 6 cores from CCD: $ccd"
    		# Pick the first two cores from CCD0 and the first six cores from the current CCD
    		picked_cores=(${CCDs[firstCCD*8]} ${CCDs[firstCCD*8+1]} ${CCDs[ccd*8]} ${CCDs[ccd*8+1]} ${CCDs[ccd*8+2]} ${CCDs[ccd*8+3]} ${CCDs[ccd*8+4]} ${CCDs[ccd*8+5]})
